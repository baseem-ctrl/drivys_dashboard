...
export function useGetTrainerReview({
  ...
  const getTheFullUrl = () => {
    ...
    return `${endpoints.reviewSchoolAdmin.getTrainerReview}?${new URLSearchParams(queryParams)}`;
  };
...
export function deleteReview({
  ...
  const URL = `${endpoints.reviewSchoolAdmin.deleteReview}${new URLSearchParams(queryParams)}`;
...
export function updateReview(body) {
  const URL = `${endpoints.reviewSchoolAdmin.updateReview}`;

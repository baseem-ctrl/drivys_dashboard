...
export default function CommissionRow({ row, selected }: Props) {
    ...
    <TableRow hover selected={selected} onClick={() => handleRowClick(row.id)}>
      ...
      <TableCell>
        <Label
          variant="soft"
          color={
            row?.booking?.driver_status === 'PENDING'
              ? 'info'
              : row?.booking?.driver_status === 'REFUNDED'
              ? 'warning'
              : row?.booking?.driver_status === 'PARTIALLY PAID'
              ? 'default'
              : row?.booking?.driver_status === 'FAILED'
              ? 'error'
              : 'success'
          }
        >
          {row?.booking?.driver_status || t('n/a')}
        </Label>
      </TableCell>
      <TableCell>

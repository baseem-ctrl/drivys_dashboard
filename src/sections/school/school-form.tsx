...
export default function SchoolCreateForm({
  ...
  const onSubmit = async (data: any) => {
    ...
    if (data.start_time) {
      const formattedTime = moment(data.start_time, ['HH:mm:ss', 'HH:mm']).format('HH:mm');
      data.append('start_time', formattedTime);
    } else {
      data.append('start_time', '');
    }
    if (data.end_time) {
      const formattedTime = moment(data.end_time, ['HH:mm:ss', 'HH:mm']).format('HH:mm');
      data.append('end_time', formattedTime);
    } else {
      ...
      <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
        ...
        <DialogContent>
          ...
          <Box display="grid" gridTemplateColumns="repeat(2, 1fr)" columnGap={2}>
            <RHFTextField name="start_time" label="Start Time" type="time" />
            <RHFTextField name="end_time" label="End Time" type="time" />

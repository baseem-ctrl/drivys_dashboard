...
export default function RevenueReportListSchoolAdminView() {
  ...
  const handleDownloadClick = async () => {
    try {
      ...
      const response = await fetch(`${baseUrl}admin/reportBySchool/revenues?${queryParams}`, {
        method: 'GET',
        headers: {
          Accept: 'text/csv',
        ...
        {viewMode === 'table' && (
          <TableContainer sx={{ position: 'relative', overflow: 'unset' }}>
            <Box style={{ display: 'flex', justifyContent: 'flex-end' }}>
              {revenueReports && Object.keys(revenueReports).length > 0 && (
                <Button
                  variant="outlined"

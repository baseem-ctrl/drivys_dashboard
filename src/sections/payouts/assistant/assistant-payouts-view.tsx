...
import AssistantPayoutFilters from './assistant-filters';
...
const PAYOUT_SORT_OPTIONS = [
  { value: 'total_paid_and_completed_booking', label: 'Paid Bookings' },
  { value: 'id', label: 'Latest' },
];
export default function AssistantPayoutPage() {
  ...
  const {
    payoutByAssistantList,
    payoutByAssistantLoading,
    payoutByAssistantError,
    payoutByAssistantEmpty,
    revalidatePayoutByAssistant,
    totalPages,
  } = useGetPayoutByAssistant({
    page: table?.page + 1,
    limit: table?.rowsPerPage,
    assistant_id: filters?.assistant_id,
  ...
  const renderLargeScreenContent = (item: any) => {
    ...
    const fields = [
      ...
      {
        label: t('amount_required_from_assistant'),
        value: (
          <>
            <span className="dirham-symbol">&#x00EA;</span>
  ...
  return (
    <Container maxWidth={settings.themeStretch ? false : 'lg'}>
      <CustomBreadcrumbs
        heading={t('assistant_payouts')}
        links={[
          { name: t('Dashboard'), href: paths.dashboard.root },
          {
            name: t('Payout'),
            href: paths.dashboard.payouts.assistant,
          },
          { name: t('assistant_payouts') },
        ]}
        sx={{
          mb: { xs: 3, md: 5 },
        }}
      ...
      <Box display="flex" flexDirection="column" gap={3}>
        {payoutByAssistantList.map((tour: any) => (
          <Card
            key={tour.id}
            sx={{
              p: 2,
